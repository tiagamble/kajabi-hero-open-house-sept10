...
const STAGE_COLORS = {Scale: 'stage-scale', Growth: 'stage-growth', Established: 'stage-established', Launch: 'stage-launch', Transition: 'stage-transition'};

function HeroCard({ hero }) {
  const [isFlipped, setIsFlipped] = useState(false);
  return (
    <div className="flip-card h-96 cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
      <div className={`flip-card-inner ${isFlipped ? 'flipped' : ''}`}>
        <div className="flip-card-front card p-6 flex flex-col items-center justify-center">
          <div className="flex items-center justify-center gap-2 mb-4">
            <div className="hero-circle bg-gray-200 flex items-center justify-center overflow-hidden">
              {hero.photo ? <img src={hero.photo} alt={hero.name} className="w-full h-full object-cover" /> : <span className="text-gray-400 text-sm">No photo</span>}
            </div>
            {hero.plusOne && (
              <div className="hero-circle-small bg-gray-200 flex items-center justify-center overflow-hidden">
                {hero.plusOne.photo ? <img src={hero.plusOne.photo} alt={hero.plusOne.name} className="w-full h-full object-cover" /> : <div className="flex items-center justify-center w-full h-full"><span className="text-gray-400 text-xs font-bold">+1</span></div>}
              </div>
            )}
          </div>
          ...
          {hero.website && <p className="text-xs text-kajabi-gold text-center mb-3">{hero.website}</p>}
        ...
        <div className="flip-card-back card p-6 flex flex-col justify-start overflow-y-auto">
          ...
          {hero.riskFlags && <><h3 className="text-sm font-bold text-kajabi-teal mb-2">Risk Flags</h3><p className="text-xs text-gray-700 line-clamp-3">{hero.riskFlags}</p></>}
...
export default function Home() {
  ...
  const filteredHeroes = useMemo(() => {
    return heroes.filter((hero) => {
      const matchesSearch = hero.name.toLowerCase().includes(search.toLowerCase()) || hero.businessName.toLowerCase().includes(search.toLowerCase()) || hero.niche.toLowerCase().includes(search.toLowerCase());
      ...
      <header className="border-b border-gray-300 bg-white sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-2 border-b border-gray-200 text-xs">
            ...
            <button onClick={() => { localStorage.removeItem('hero-auth-sept10'); window.location.href = '/'; }} className="text-gray-600 hover:text-gray-900">Logout</button>
          </div>
          <div className="py-4 mb-4">
            ...
            <div className="mb-4">
              <input type="text" placeholder="Search by name, business, or niche..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-kajabi-teal" />
            </div>
            <div className="flex items-center gap-2 overflow-x-auto pb-2">
              {STAGES.map((stage) => (<button key={stage} onClick={() => setSelectedStage(stage)} className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all ${selectedStage === stage ? (stage === 'All Stages' ? 'bg-kajabi-gold text-white' : 'bg-kajabi-sage text-white') : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>{stage}</button>))}
            </div>
          </div>
        </div>
      </header>
